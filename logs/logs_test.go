...
func TestNew(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "ok",
			args: args{ctx: context.Background(), logTag: logTag, logger: logger.Logger},
			want: &Logs{ctx: context.Background(), logTag: logTag, logger: logger.Logger},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := New(tt.args.ctx, tt.args.logTag, tt.args.logger); !reflect.DeepEqual(got, tt.want) {
